...
func (dc dashContainer) NewDashboard(variables url.Values) Dashboard {
	...
	dash.ReportFooter = sanitizeLaTexInput(GetReportFooter())
...
// GetReportFooter will replace the footer text of report if env is defined
func GetReportFooter() string {
	reportFooter, ok := os.LookupEnv("FOOTER")
	if !ok {
		reportFooter = "MayaData Inc."
	}
	return reportFooter
}
